...
class Board(object):
    ...
    def is_valid_attack_path(self, path):
        ...
        '''
        if len(path) >= 2 and self.is_valid_path(path):
            owner = self.owner(path[0])
            for i in range(1, len(path)):
                if self.owner(path[i]) == owner:
                    return False
            return True
        else:
            return False
    ...
    def cost_of_attack_path(self, path):
        ...
        if self.is_valid_path(path):
            for territory in path[1:]:
                num_armies += self.armies(territory)
            return num_armies
        else:
            return False
    ...
    def shortest_path(self, source, target):
        ...
        stack.append(source)
        shortest_path = deque([])
        shortest_path.append(stack)
        if source == target:
            return stack
        while len(shortest_path) != 0:
            dequeued_path = shortest_path.popleft()
            for territory in list(territories):
                neighbors = list(self.neighbors(dequeued_path[-1]))
                neighbors = [neighbor.territory_id for neighbor in neighbors]
                if territory in neighbors:
                    if territory == target:
                        dequeued_path.append(territory)
                        return dequeued_path
                    copy_path = dequeued_path[:]
                    copy_path.append(territory)
                    shortest_path.append(copy_path)
    ...
    def can_fortify(self, source, target):
        ...
        while len(shortest_path) != 0:
            ...
            for territory in list(territories):
                ...
                neighbors = [
                    neighbor for neighbor in neighbors if self.owner(neighbor) == owner]
                if territory in neighbors:
                    if territory == target:
                        dequeued_path.append(territory)
                        return dequeued_path
    ...
    def cheapest_attack_path(self, source, target):
        ...
        if source == target:
            return None
        while not q.empty():
            old_priority, current_territory = q.get()
            ...
            for territory in neighbors:
                if territory not in visited and self.owner(
                        source) != self.owner(territory):
                    copy = dictionary[current_territory].copy()
                    copy.append(territory)
                    priority = old_priority + self.armies(territory)
                    if territory not in [item[1] for item in q.queue]:
                        dictionary[territory] = copy
                        q.put((priority, territory))
                    else:
                        if priority < min(
                                [item[0] for item in q.queue if item[1] == territory]):
                            dictionary[territory] = copy
                            for i, (p, t) in q.queue:
                                if t == territory:
                                    q.queue[i] = (priority, territory)
    ...
    def can_attack(self, source, target):
        ...
        '''
        # Use cheapest_attack_path and use shortest attack path
        # check if either are valid paths; if not return False else return True
    ...
    def continent(self, continent_id):
        ...
        """
        return (
            t for t in self.data if t.territory_id in risk.definitions.continent_territories[continent_id])
    ...
    def n_continents(self, player_id):
        ...
        """
        return len([continent_id for continent_id in range(
            6) if self.owns_continent(player_id, continent_id)])
    ...
    def possible_attacks(self, player_id):
        ...
        return [
            Move(
                ...
                to_t.territory_id,
                to_t.player_id,
                to_t.armies) for from_t in self.mobile(player_id) for to_t in self.hostile_neighbors(
                from_t.territory_id)]
    ...
    def possible_fortifications(self, player_id):
        ...
        return [
            Move(
                ...
                to_t.territory_id,
                to_t.player_id,
                to_t.armies) for from_t in self.mobile(player_id) for to_t in self.friendly_neighbors(
                from_t.territory_id)]
    ...
    def attack(self, from_territory, to_territory, attackers):
        ...
        if to_territory not in [
            tid for (
                tid,
                _,
                _) in self.hostile_neighbors(from_territory)]:
            raise ValueError(
